describe("Albums", () => {
  ...
  context("add new album", () => {
    it("adds new album to the album list", () => {
      ...
      cy.get('[data-cy="upload-cover-text"]').should(
        "contain.text",
        "Add cover",
      );
      cy.get("input[type=file]").selectFile(
        "db/cover_images/happier_than_ever.png",
        { force: true },
      );
      cy.get('[data-cy="upload-cover-text"]').should(
        "contain.text",
  ...
  it("updates existing album to the album list", () => {
    ...
    cy.get('[data-cy="album-name-input"]').type(
      "{selectAll}Updated Album name",
    );
    cy.get('[data-cy="track-item"]').not(".hidden").should("have.length", 3);
    cy.get('[data-cy="add-track-button"]').click();
    ...
    cy.get('[data-cy="alb-title-1"]').should(
      "contain.text",
      "Updated Album name",
    );
